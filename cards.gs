...
const HDFCUPISearch = "((from:alerts@hdfcbank.net AND subject:You have done a UPI txn. Check details!) OR (from:hdfcbank.net AND subject:View: Account update for your HDFC Bank A/c))"
const HDFCUPI = new CardInfo(HDFCUPISearch, HDFCUPIRegex, 7, "DD-MM-YY", 5 , 1);

const AxisBankCCRegex = /Card no.\s(XX\d+)\sfor\sINR\s(\d+(.\d+))?\sat\s+(.+?)\son\s*(\d+-\d+-\d+\s\d+:\d+:\d+)/;
const AxisBankCCSearch = "from:alerts@axisbank.com AND subject:Transaction alert on Axis Bank Credit Card no"
const AxisCC = new CardInfo(AxisBankCCSearch, AxisBankCCRegex, 5, "DD-MM-YY", 4 , 2);
...
const IDFCCC = new CardInfo(IDFCCCSearch, IDFCCCRegex, 4, "DD-MM-YYYY", 3 , 1);

const ICICICCRegex = /a transaction of Rs ([0-9,.]+) on ([A-Za-z0-9 @&$-_]+). Info: (.*?). The Available Credit Limit /;
...
const CITICCSearch = "from:CitiAlert.India@citicorp.com AND subject:Transaction confirmation on your Citibank credit card"
